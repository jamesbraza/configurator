...
4. Download delta's themes, which define `zebra-dark`:
   ...
   ```shell
   curl -s --create-dirs -o ~/.config/git/themes.gitconfig \
     https://raw.githubusercontent.com/dandavison/delta/main/themes.gitconfig
   ```

5. `git config --global user.name "..."` and `git config --global user.email "..."`.
6. Optionally `herdr plugin install qu8n/herdr-automatic-rename --yes`;
   the `alt+shift+r` binding in `herdr`'s config is inert without it.
7. `bunx -y ccstatusline@latest` and choose "Pinned global install",
   which writes `statusLine` into Claude Code's settings.
8. `herdr integration install claude`, after copying Claude Code's settings,
